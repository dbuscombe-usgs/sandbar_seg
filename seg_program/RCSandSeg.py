...
def gui():
    ...
    def _qfind():
        ...
        if self.datloaded == 0:
           ...
           print("Time lag in hours downstream from nearest gage: "+str(self.lag_hrs))
           self.datloaded = 1
        ...
        root = Tk()
        root.wm_title("Filtered Images")
        S = Scrollbar(root)
        T = Text(root, height=40, width=160, wrap=WORD)
        S.pack(side=RIGHT, fill=Y)
        T.pack(side=LEFT, fill=Y)
        S.config(command=T.yview)
        T.config(yscrollcommand=S.set)
