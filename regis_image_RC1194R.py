...
if __name__ == '__main__':
   ...
   master = '/run/media/dbuscombe/MASTER/GCMRC/SANDBAR_REMOTECAMERAS/RC1194R/deployment7/RC1194R_20141004_1359.JPG'

   direc = '/run/media/dbuscombe/MASTER/GCMRC/SANDBAR_REMOTECAMERAS/RC1194R/deployment7/'
   filenames = sorted(filter(os.listdir(direc), '*.[Jj][Pp][Gg]'))

   outdirec = '/run/media/dbuscombe/MASTER/GCMRC/SANDBAR_REMOTECAMERAS/RC1194R_regis/deployment7/'
   ...
   im2.save('temp/2e.pgm')
   im2 = np.asarray(im2)
   process_image('temp/2e.pgm', 'temp/2e.key')
   key2 = read_features_from_file('temp/2e.key')
