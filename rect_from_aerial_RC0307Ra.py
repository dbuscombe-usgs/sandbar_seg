...
xyz = np.genfromtxt('RC0307Rf_surveydata/030_121004_GRID.TXT', delimiter='\t')
...
#plt.plot(xyz[:,0],xyz[:,1],'.', markersize=2)
...
pts = np.array([[  1.89454032e+02,   5.46988508e+02],
...
pickle.dump( {'trans':homo[0], 'dsize':tuple(np.ceil(np.max(pts2, axis=0)*1.2).astype('int')), 'impts':impts, 'mappts':pts2}, open( "RC0307Rf_homo_trans.p", "wb" ) )
...
plt.savefig('RC0307Rf_rect_image_ex.png')
plt.close()
